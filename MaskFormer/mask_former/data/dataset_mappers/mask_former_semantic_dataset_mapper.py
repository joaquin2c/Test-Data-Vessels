...
class MaskFormerSemanticDatasetMapperLITS:
    ...
    def __call__(self, dataset_dict):
        ...
        image = (np.load(dataset_dict["file_name"])).astype(np.float32) # (h,w) RGB - float32
        image = image - 1024.0
        ...
        image = (image - img_min) / (img_max - img_min) # [0,1] -> Entre 0 y 1 para aumentación de datos Y/O normalizar respecto a pesos de Imagenet
        ...
        if "sem_seg_file_name" in dataset_dict:
            # PyTorch transformation not implemented for uint16, so converting it to double first
            sem_seg_gt = np.load(dataset_dict.pop("sem_seg_file_name")).astype("double")
        else:
            sem_seg_gt = None
        ...
        image_pre = image.transpose(2, 0, 1).astype(np.float32) 
        #sem_seg_gt = sem_seg_gt.astype('float32') / 255
        #sem_seg_gt = sem_seg_gt.transpose(2, 0, 1)

        image = torch.tensor(image_pre)
